...
from omni.isaac.orbit.assets import Articulation, FixedObject, RigidObject
...
def reset_fixed_body_root_state_uniform(
    env: BaseEnv,
    env_ids: torch.Tensor,
    pose_range: dict[str, tuple[float, float]],
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
...
def reset_scene_to_default(env: BaseEnv, env_ids: torch.Tensor):
    ...
    for deformable_object in env.scene.deformable_objects.values():
        # obtain default nodal_poses
        default_nodal_state = deformable_object.data.default_nodal_state_w[env_ids].clone()
        # set into the physics simulation
        deformable_object.write_root_pos_to_sim(
            default_nodal_state[:, : default_nodal_state.size(1) // 2, :], env_ids=env_ids
        )
        deformable_object.write_root_velocity_to_sim(
            torch.zeros_like(default_nodal_state[:, default_nodal_state.size(1) // 2 :, :]), env_ids=env_ids
        )
