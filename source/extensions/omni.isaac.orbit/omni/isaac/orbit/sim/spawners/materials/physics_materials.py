...
@clone
def spawn_particle_system_material(prim_path: str, cfg: physics_materials_cfg.ParticleMaterialCfg) -> Usd.Prim:
    """Create material with particle-system physics properties.
    Particle system materials are used to define the physical properties to particle systems. These
    include the friction and particle system properties. For more information on
    particle system material, please refer to the `documentation on PxParticleBase <https://nvidia-omniverse.github.io/PhysX/physx/5.2.1/_build/physx/latest/class_px_particle_base.html>`_.
    .. note::
        This function is decorated with :func:`clone` that resolves prim path into list of paths
        if the input prim path is a regex pattern. This is done to support spawning multiple assets
        from a single and cloning the USD prim at the given path expression.
    Args:
        ...
        cfg: The configuration for the physics material.
    Returns:
        The spawned particle system material prim.
    Raises:
        ValueError:  When a prim already exists at the specified prim path and is not a material.
    """
    # create material prim if no prim exists
    ...
    usd_physics_material_api = PhysxSchema.PhysxPBDMaterialAPI(prim)
    if not usd_physics_material_api:
        usd_physics_material_api = PhysxSchema.PhysxPBDMaterialAPI.Apply(prim)
