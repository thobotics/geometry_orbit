...
@clone
def spawn_cloth_from_usd(
    prim_path: str,
    cfg: from_files_cfg.ClothUsdFileCfg,
    translation: tuple[float, float, float] | None = None,
    orientation: tuple[float, float, float, float] | None = None,
) -> Usd.Prim:
    """Spawn a cloth asset from a USD file and override the settings with the given config.
    ...
    return _spawn_cloth_from_usd_file(prim_path, cfg.usd_path, cfg, translation, orientation)
...
def _spawn_cloth_from_usd_file(
    prim_path: str,
    usd_path: str,
    cfg: from_files_cfg.ClothUsdFileCfg,
    translation: tuple[float, float, float] | None = None,
    ...
    if cfg.mass_props is not None:
        predicate = lambda path: prim_utils.get_prim_type_name(path) == "Mesh"
        mesh_prim = prim_utils.get_first_matching_child_prim(prim_path, predicate)
