...
from pxr import PhysxSchema, Sdf, Semantics, Usd, UsdGeom, UsdPhysics, UsdShade, Gf
...
"""
Apply PhysX attachment.
"""


def apply_body_attachment(prim_path, attachment_path, attachment_name, attachment_props):
    """Apply a PhysX attachment to a prim.

    Args:
        prim_path: The prim path to apply the attachment to.
        attachment_path: The prim path of the attachment to apply.
        attachment_props: The attachment properties to apply.
    """
    # get current stage
    stage = stage_utils.get_current_stage()

    prims = find_matching_prims(prim_path, stage)
    attachment_prims = find_matching_prims(attachment_path, stage)

    for prim, attachment_prim in zip(prims, attachment_prims):
        mesh_prim = get_first_matching_child_prim(prim.GetPath(), lambda prim: prim.GetTypeName() == "Mesh")
        attached_mesh_prim = get_first_matching_child_prim(
            attachment_prim.GetPath(), lambda prim: prim.GetTypeName() in ["Cube", "Mesh"]
        )
        attachment_path = mesh_prim.GetPath().AppendElementString(attachment_name)
        physx_attachment = PhysxSchema.PhysxPhysicsAttachment.Define(stage, attachment_path)
        physx_attachment.GetActor0Rel().SetTargets([attached_mesh_prim.GetPath()])
        physx_attachment.GetActor1Rel().SetTargets([mesh_prim.GetPath()])
        PhysxSchema.PhysxAutoAttachmentAPI.Apply(physx_attachment.GetPrim())

        # TODO: set attachment properties


def apply_joint_attachment(name, joint_type, parent_prim_path, child_prim_path, local_pos0, local_pos1):
    """Apply a PhysX joint attachment to a prim.
    ...
    parent_prims = find_matching_prims(parent_prim_path, stage)
    child_prims = find_matching_prims(child_prim_path, stage)

    for i, (parent_prim, child_prim) in enumerate(zip(parent_prims, child_prims)):
        joint_path = parent_prim.GetPath().AppendElementString(name)
