...
class InteractiveScene:
    ...
    fixed_objects: dict[str, FixedObject] = dict()
    """A dictionary of fixed objects in the scene."""
    deformable_objects: dict[str, DeformableObject] = dict()
    """A dictionary of deformable objects in the scene."""
    cloth_objects: dict[str, ClothObject] = dict()
    """A dictionary of cloth objects in the scene."""
    rope_objects: dict[str, Rope] = dict()
    """A dictionary of rope objects in the scene."""
    ...
    def reset(self, env_ids: Sequence[int] | None = None):
        ...
        for fixed_object in self.fixed_objects.values():
            fixed_object.reset(env_ids)
        for deformable_object in self.deformable_objects.values():
            deformable_object.reset(env_ids)
        for cloth_object in self.cloth_objects.values():
            cloth_object.reset(env_ids)
        for rope in self.rope_objects.values():
            rope.reset(env_ids)
        # -- sensors
    ...
    def write_data_to_sim(self):
        ...
        for fixed_object in self.fixed_objects.values():
            fixed_object.write_data_to_sim()
        for deformable_object in self.deformable_objects.values():
            deformable_object.write_data_to_sim()
        for cloth_object in self.cloth_objects.values():
            cloth_object.write_data_to_sim()
        for rope in self.rope_objects.values():
            rope.write_data_to_sim()
        # -- flush physics sim view if called in extension mode
        # this is needed when using PhysX GPU pipeline since the data needs to be sent to the underlying
        # PhysX buffers that might live on a separate device
        # note: In standalone mode, this method is called in the `step()` method of the simulation context.
    ...
    def _add_entities_from_cfg(self):
        ...
        for asset_name, asset_cfg in self.cfg.__dict__.items():
            ...
            if isinstance(asset_cfg, TerrainImporterCfg):
                ...
                if (
                    isinstance(asset_cfg, RigidObjectWithAttachmentCfg)
                    and asset_cfg.attachments is not None
                    and len(asset_cfg.attachments) > 0
                ):
                    for attachment in asset_cfg.attachments:
                        attachment.prim_path = attachment.prim_path.format(ENV_REGEX_NS=self.env_regex_ns)
                self.rigid_objects[asset_name] = asset_cfg.class_type(asset_cfg)
            elif isinstance(asset_cfg, FixedObjectCfg):
                self.fixed_objects[asset_name] = asset_cfg.class_type(asset_cfg)
            elif isinstance(asset_cfg, DeformableObjectCfg):
                # Update attachments' regex name space for DeformableObject
                if asset_cfg.attachments is not None and len(asset_cfg.attachments) > 0:
                    for attachment in asset_cfg.attachments:
                        attachment.prim_path = attachment.prim_path.format(ENV_REGEX_NS=self.env_regex_ns)
                self.deformable_objects[asset_name] = asset_cfg.class_type(asset_cfg)
            elif isinstance(asset_cfg, ClothObjectCfg):
                # Update attachments' regex name space for ClothObject
                if asset_cfg.attachments is not None and len(asset_cfg.attachments) > 0:
                    for attachment in asset_cfg.attachments:
                        attachment.prim_path = attachment.prim_path.format(ENV_REGEX_NS=self.env_regex_ns)

                self.cloth_objects[asset_name] = asset_cfg.class_type(asset_cfg)
            elif isinstance(asset_cfg, RopeCfg):
                # Update attachments' regex name space for RopeObject
                if asset_cfg.attachments is not None and len(asset_cfg.attachments) > 0:
                    for attachment in asset_cfg.attachments:
                        attachment.prim_path = attachment.prim_path.format(ENV_REGEX_NS=self.env_regex_ns)

                self.rope_objects[asset_name] = asset_cfg.class_type(asset_cfg)
